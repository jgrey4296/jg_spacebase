...
function HostileDockingEvent.allowEvent(nPopulation, nElapsedTime)
    return not g_Config:getConfigValue('disable_hostile_events') and not g_Config:getConfigValue('disable_hostiles') and not g_Config:getConfigValue('disable_docking') and (nPopulation > 4 or GameRules.elapsedTime > 60*10)
end
